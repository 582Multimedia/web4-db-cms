...
You should now be able to access this new custom post type directly using the wordpress API using this path:

`your-wordpress-path`/wp-json/wp/v2/movies

where `your-wordpress-path` is the path on your server for the wordpress you are working on.

For my example, you can visit my movies API path here:
[https://ngy.582mi.com/headless/wp-json/wp/v2/movies](https://ngy.582mi.com/headless/wp-json/wp/v2/movies)

Make a new component called `WpHeadlessBasics.vue` and use refer back to `WpApiBasics.vue` to try and access and display the data coming from your new custom post type.

> [!TIP]
> Explore the array you're getting from this custom post type.
>
> If you followed all the steps, for each of the posts you made, you should be able to find a property called `acf` that contains the new custom post data you entered as an object.
